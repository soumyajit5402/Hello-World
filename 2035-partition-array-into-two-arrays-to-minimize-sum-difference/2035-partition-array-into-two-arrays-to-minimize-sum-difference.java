class Solution {
    public int minimumDifference(int[] nums) {
        TreeSet<Integer>[] containerLeft = new TreeSet[1 + (nums.length >> 1)];
        TreeSet<Integer>[] containerRight = new TreeSet[1 + (nums.length >> 1)];
        ...
        for (int leftLen = 0; leftLen <= nums.length/2; leftLen++) {
            ...
            TreeSet<Integer> rightVals = containerRight[rightLen];
    ...
    private void findAllSums(int[] arr, int left, int right, int count, int sum, TreeSet<Integer>[] container) {
        if (container[count] == null) container[count] = new TreeSet<>();
